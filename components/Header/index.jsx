...
export default function Header() {
  ...
  return (
    <>
      <header className="h-20 w-full  flex px-2 md:px-10 fixed top-0 z-10 bg-white shadow-lg">
        <div className="w-full h-full flex justify-between items-center">
          <div>
            <Link href={'/'}>
              <Image src="/logo3.svg" alt="" width={200} height={200} />
            </Link>
          </div>
          {isloggedIn && user && (
        ...
        <div className="bg-white border-solid border-[1px] border-slate-500 top-[5rem]  right-[2.5rem] float-right   shadow-xl z-50 rounded-md fixed">
          <div className="flex flex-col w-full p-4">
            ...
            {user.permission === 'admin' && (
              <>
                <span className="font-light text-optionB-gray-dark">
                  administrador
                </span>
            ...
            {user.permission === 'user' && (
              <MenuLink href={`/user/minhas-reservas/${user.id}`}>
                Minhas reservas
              </MenuLink>
            )}
            <span
              className="rounded-md bg-red-500 hover:bg-red-400  text-white font-medium flex justify-center"
              onClick={signOut}
            >
              Logout
            </span>
