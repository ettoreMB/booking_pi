...
export default function Card({ produto }) {
  ...
  const text =
    'No coração de San Telmo, desfrute de uma pousada inspirada nas paixões de Buenos Aires. com 2 piscinas impressionantes, uma no terraço e outra ao ar livre; quartos privados alguns ... ms...'
  const textPreview = text.substring(0, 150)

  return (
    <div className=" h-[500px] md:h-72 w-full  flex flex-1 p-1">
        ...
        <div className="h-1/2 md:h-full flex flex-col p-3 w-full md:w-1/2">
          <div className="flex justify-between mb-4">
            <div className="flex flex-col">
              <div className="flex text-sm text-slate-500 font-bold gap-2">
                <span>HOTEL</span>
                <span className="">starts</span>
              </div>
              <h2 className="text-2xl font-bold text-optionB-gray-dark">
                {produto.name}
              </h2>
            </div>
          ...
          <div className="mb-4 flex items-center justify-start text-sm gap-1">
            <Image
              ...
              width={16}
            />
            <span>A {Math.floor(Math.random() * 120)} m do centro - </span>
            <Link href={`/`} className="uppercase text-optionB-main text-end">
              MOSTRAR NO MAPA
            </Link>
          </div>
          <div className="break-all mb-4 h-9 flex flex-1 flex-col">
            <p className="break-all overflow-hidden ">
              {textIsBiggerThanLimit ? textPreview : text}
            </p>
            {textIsBiggerThanLimit && (
              <Link href={`/produto/${produto.id}`} className="text-blue-400">
                ...ler mais
              </Link>
            )}
          </div>
          <Suspense fallback={<Loading />}>
            <ButtonLink href={`/produto/${produto.id}`} title={'Ver mais'} />
          </Suspense>
        </div>
      </div>
    </div>
